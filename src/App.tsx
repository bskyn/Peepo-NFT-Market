...
import { ErrorContainer } from './components/SearchError/ErrorContainer';
...
function App() {
  ...
  const { data: collectionData, status: collectionStatus } = useFetchCollection(
  ...
  } else if (collectionStatus === 'error' || assetsStatus === 'error') {
    return (
      <SearchError
        searchVal={searchVal}
        onChange={onChange}
  ...
  return (
    <>
      ...
      {assetsData?.pages[0]?.assets.length === 0 ? (
        <ErrorContainer>
          This contract address does not have any NFT assets
        </ErrorContainer>
      ) : (
        <Page>
